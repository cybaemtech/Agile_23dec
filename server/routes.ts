...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Helper function to calculate total estimated hours for hierarchical work items
  const calculateHierarchicalEstimates = async (items: any[]) => {
    const itemsWithEstimates = await Promise.all(
      items.map(async (item) => {
        let calculatedEstimate = item.estimate ? parseFloat(item.estimate.toString()) : 0;
        
        // If it's an EPIC or FEATURE, sum up child estimates
        if (item.type === 'EPIC' || item.type === 'FEATURE') {
          const children = await storage.getWorkItemsByParent(item.id);
          if (children.length > 0) {
            const childrenWithEstimates = await calculateHierarchicalEstimates(children);
            const childTotal = childrenWithEstimates.reduce((sum, child) => 
              sum + (child.calculatedEstimate || 0), 0
            );
            calculatedEstimate = Math.max(childTotal, calculatedEstimate);
          }
        }
        
        return { ...item, calculatedEstimate };
      })
    );
  ...
  app.get('/api/projects/:projectId/work-items', canAccessProject, async (req, res) => {
    try {
      ...
      // Get only top-level items (EPIC, FEATURE) for initial load
      const topLevelItems = workItems.filter(item => !item.parentId);
      const itemsWithEstimates = await calculateHierarchicalEstimates(topLevelItems);
      
      res.json(itemsWithEstimates);
    } catch (error) {
  ...
  app.get('/api/work-items/:parentId/children', async (req, res) => {
    try {
      ...
      // Calculate estimated hours for each child
      const childrenWithEstimates = await Promise.all(
        children.map(async (child) => {
          let calculatedEstimate = child.estimate ? parseFloat(child.estimate.toString()) : 0;
          
          // If child is EPIC or FEATURE, sum its children
          if (child.type === 'EPIC' || child.type === 'FEATURE') {
            const grandchildren = await storage.getWorkItemsByParent(child.id);
            if (grandchildren.length > 0) {
              const grandchildTotal = grandchildren.reduce((sum, gc) => 
                sum + (gc.estimate ? parseFloat(gc.estimate.toString()) : 0), 0
              );
              calculatedEstimate = Math.max(grandchildTotal, calculatedEstimate);
            }
          }
          // If child is STORY, sum its tasks
          else if (child.type === 'STORY') {
            const tasks = await storage.getWorkItemsByParent(child.id);
            if (tasks.length > 0) {
              const taskTotal = tasks.reduce((sum, task) => 
                sum + (task.estimate ? parseFloat(task.estimate.toString()) : 0), 0
              );
              calculatedEstimate = Math.max(taskTotal, calculatedEstimate);
            }
          }
          
          return { ...child, calculatedEstimate };
        })
      );
      
      res.json(childrenWithEstimates);
    } catch (error) {
  ...
  app.patch('/api/work-items/:id', canManageWorkItemType(['EPIC', 'FEATURE', 'STORY', 'TASK', 'BUG']), async (req, res) => {
    try {
      ...
      // Additional permission check for regular users
      if (currentUser.role === "USER") {
        // Regular users can only edit work items they are assigned to
        if (existingWorkItem.assigneeId !== userId) {
          return res.status(403).json({ 
            message: "You can only edit work items assigned to you" 
          });
        }
      }
      ...
      if (currentUser.role === "USER") {
        ...
        if (existingWorkItem.type === 'EPIC' || existingWorkItem.type === 'FEATURE') {
          return res.status(403).json({ 
            message: "Only Admin and Scrum Master can edit EPIC and FEATURE items" 
          });
        }
      ...
      // ADMIN and SCRUM_MASTER can edit any work items (already validated by middleware)
  ...
  // File attachment endpoint for screenshots and other files
  app.post('/api/attachments', async (req, res) => {
    try {
      ...
      const userId = (req.session as any).userId || 1; // Default to user 1 if not in session
      ...
      // For now, just return success as we'll store file references
      // In production, you'd handle actual file uploads to cloud storage
      res.status(201).json({ 
      ...
      res.status(500).json({ message: "Error processing attachment" });
    }
  });
