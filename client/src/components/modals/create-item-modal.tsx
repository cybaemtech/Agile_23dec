...
export function CreateItemModal({
  ...
  // Initialize with correct type when modal opens - only run once per modal open
  React.useEffect(() => {
  ...
  }, [isOpen, currentUser, defaultType, form, preselectedType, preselectedParent, allowedTypes, isInitialized]);
  ...
  const onSubmit = async (data: WorkItemFormValues) => {
    ...
    try {
      ...
      // Upload screenshot if provided and work item is a BUG
      if (selectedFile && data.type === 'BUG' && response.id) {
        const formData = new FormData();
        formData.append('file', selectedFile);
        formData.append('workItemId', response.id.toString());
        
        try {
          await apiRequest("POST", "/attachments", formData);
        } catch (uploadError) {
          console.error("Error uploading screenshot:", uploadError);
          // Don't fail the entire operation if screenshot upload fails
          toast({
            title: "Warning",
            description: "Item created, but screenshot upload failed.",
            variant: "default",
          });
        }
      }
